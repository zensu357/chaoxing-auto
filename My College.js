// ==UserScript==
// @name         我的大学我做主🦄️ (自动答题｜AI大模型搜题｜视频倍速｜截屏搜题)
// @namespace    https://github.com/zensu357/clean-ai-assistant/
// @version      0.0.1
// @description  支持自定义接入通用大模型 API (DeepSeek/OpenAI/通义千问/Ollama等)；深度穿透多层 iframe 全自动答题与高兼容模拟勾选；支持暂停、继续、停止与实时运行日志；兼容超星、智慧树、中国大学MOOC、云班课、雨课堂、国开、智慧职教等平台。
// @author       zensu357
// @match        *://*.chaoxing.com/*
// @match        *://*.treewises.com/*
...
;(function(_this) {
    var shadowContent, aner, div_zhu, div_set, find_input;
    ...
    function MyPage(menu){
        ...
        let exitElement;
        // 纯净版修复：无论是否为特定网课学习页面，只要匹配域名，悬浮球永远默认显示，保证用户随时可用 AI 解题与设置！
        const show = "block";
        exitElement = this.initMenu("block");
        if(this.api){
            this.prival_global();
        } else {
            console.log("%c【网课助手纯净版】已就绪 (通用辅助模式，AI搜题与设置随时可用)", "color:#6366f1;font-weight:bold;");
        }
        setInterval(() => {
    ...
    */
    class chaoxin_api{
        ...
        encode_font(){
            ...
            // 解析font-cxsecret字体
            var fontMatch = $tip.text().match(/base64,([^'"\)\s]+)/); if(!fontMatch) return; var font = fontMatch[1];
            font = Typr.parse(base64ToUint8Array(font))[0];

            // 匹配解密字体
            var tableRaw = GM_getResourceText('Table'); if(!tableRaw) return; var table = JSON.parse(tableRaw);
            var match = {};
        ...
        async init_button(){
            if(this.config.c === "res"||this.config.front_url[this.config.front_url.length-1] === "course-learning"){
                ...
                const targetFrame = (typeof document !== 'undefined' && document.querySelector) ? document.querySelector('iframe') : null;
                const onFrameReady = () => {
                ...
                if (targetFrame) {
                    targetFrame.addEventListener('load', onFrameReady);
                }
                setTimeout(onFrameReady, 1000);
    ...
    MyPage.prototype.initMenu = function(show){
        ...
        */
        const styleTag = `
            #${menu.id}{
                --zen-primary: #4f46e5;
                --zen-primary-hover: #4338ca;
                --zen-primary-light: #eef2ff;
                --zen-cyan: #06b6d4;
                --zen-text-main: #0f172a;
                --zen-text-muted: #64748b;
                --zen-text-sub: #94a3b8;
                --zen-bg-glass: rgba(255, 255, 255, 0.94);
                --zen-border: rgba(226, 232, 240, 0.85);
                --zen-shadow-sm: 0 2px 8px -1px rgba(15, 23, 42, 0.06);
                --zen-shadow-md: 0 12px 28px -6px rgba(15, 23, 42, 0.12), 0 4px 12px -2px rgba(15, 23, 42, 0.04);
                --zen-shadow-lg: 0 20px 45px -10px rgba(15, 23, 42, 0.16), 0 6px 16px -4px rgba(15, 23, 42, 0.06);
            ...
            #${menu.id} #zhu_${menu.id}:hover{
                box-shadow: 0 14px 30px -4px rgba(15, 23, 42, 0.16), 0 6px 12px -2px rgba(15, 23, 42, 0.08);
            }
            #${menu.id} #x_set{
                width: 32px;
                height: 32px;
            ...
            #${menu.id} #zhu_${menu.id} button:active{
            ...
            #${menu.id} .drawer::-webkit-scrollbar{ width: 5px; }
            ...
            #${menu.id} .search-row:focus-within{
                border-color: #818cf8;
            ...
            #${menu.id} .drawer input::placeholder{ color: #94a3b8; }
            ...
            #${menu.id} .drawer .auto-answer-btn{
                display: flex;
                align-items: center;
                justify-content: center;
                gap: 6px;
            ...
            #${menu.id} .drawer .auto-answer-btn:hover{
            ...
            /* 自动答题控制台 */
            #${menu.id} #aner.auto-mode{
                width: 380px !important;
                max-width: min(420px, calc(100vw - 20px)) !important;
                max-height: min(600px, calc(100vh - 20px)) !important;
                padding: 14px;
            }
            #${menu.id} .auto-console{
                display: flex;
                flex-direction: column;
                gap: 10px;
            }
            #${menu.id} .auto-console-header{
                display: flex;
                align-items: center;
                justify-content: space-between;
                padding-bottom: 8px;
            ...
            #${menu.id} .auto-badge.running{ background: #dcfce7; color: #15803d; }
            #${menu.id} .auto-badge.paused{ background: #fef9c3; color: #a16207; }
            #${menu.id} .auto-badge.stopped{ background: #fee2e2; color: #b91c1c; }
            #${menu.id} .auto-badge.completed{ background: #ede9fe; color: #6d28d9; }
            #${menu.id} .auto-back-btn{
                background: #f1f5f9;
                border: 1px solid #e2e8f0;
                border-radius: 7px;
                padding: 3px 9px;
            ...
            #${menu.id} .auto-back-btn:hover{
                background: #e2e8f0;
                color: #0f172a;
            }

            #${menu.id} .auto-stat-grid{
                display: grid;
                grid-template-columns: repeat(4, 1fr);
                gap: 6px;
            }
            #${menu.id} .auto-stat-card{
                background: #f8fafc;
                border: 1px solid #e2e8f0;
                border-radius: 8px;
                padding: 7px 4px;
            ...
            #${menu.id} .auto-stat-card .num{
                font-size: 14px;
                font-weight: 800;
                color: #0f172a;
            }
            #${menu.id} .auto-stat-card .label{
                font-size: 10px;
                color: #64748b;
                margin-top: 1px;
            }

            #${menu.id} .auto-progress-bar-wrap{
                height: 6px;
                background: #e2e8f0;
                border-radius: 999px;
                overflow: hidden;
            }
            #${menu.id} .auto-progress-bar-fill{
                height: 100%;
            ...
            #${menu.id} .btn-pause{ background: #fef3c7; color: #92400e; border: 1px solid #fde68a; }
            #${menu.id} .btn-pause:hover{ background: #fde68a; }
            #${menu.id} .btn-resume{ background: #dcfce7; color: #166534; border: 1px solid #bbf7d0; }
            #${menu.id} .btn-resume:hover{ background: #bbf7d0; }
            #${menu.id} .btn-stop{ background: #fee2e2; color: #991b1b; border: 1px solid #fecaca; }
            #${menu.id} .btn-stop:hover{ background: #fecaca; }
            #${menu.id} .btn-restart{ background: #e0e7ff; color: #3730a3; border: 1px solid #c7d2fe; }
            #${menu.id} .btn-restart:hover{ background: #c7d2fe; }
            #${menu.id} .btn-clear{ flex: 0.6; background: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; }
            #${menu.id} .btn-clear:hover{ background: #e2e8f0; }

            /* 终端标题栏与深色控制台 */
            #${menu.id} .terminal-bar{
                display: flex;
                align-items: center;
                justify-content: space-between;
                padding: 5px 8px;
            ...
            #${menu.id} .terminal-dots{
                display: flex;
                align-items: center;
                gap: 5px;
            }
            #${menu.id} .terminal-dots .dot{
                width: 8px;
                height: 8px;
                border-radius: 50%;
                display: inline-block;
            }
            #${menu.id} .terminal-dots .dot.red{ background: #ef4444; }
            #${menu.id} .terminal-dots .dot.yellow{ background: #f59e0b; }
            #${menu.id} .terminal-dots .dot.green{ background: #10b981; }
            #${menu.id} .terminal-title{
                font-size: 10px;
                color: #94a3b8;
                font-weight: 600;
                margin-left: 4px;
            }
            #${menu.id} .autoscroll-label{
                font-size: 10px;
                color: #94a3b8;
                display: flex;
                align-items: center;
            ...
            #${menu.id} .auto-log-terminal::-webkit-scrollbar{ width: 5px; }
            #${menu.id} .auto-log-terminal::-webkit-scrollbar-thumb{ background: #334155; border-radius: 999px; }
            #${menu.id} .log-item{ margin-bottom: 3px; word-break: break-all; }
            #${menu.id} .log-time{ color: #64748b; margin-right: 4px; }
            #${menu.id} .log-tag-sys{ color: #38bdf8; font-weight: 700; }
            #${menu.id} .log-tag-succ{ color: #4ade80; font-weight: 700; }
            #${menu.id} .log-tag-warn{ color: #facc15; font-weight: 700; }
            #${menu.id} .log-tag-err{ color: #f87171; font-weight: 700; }
            #${menu.id} .log-tag-ai{ color: #c084fc; font-weight: 700; }

            /* 思考过程折叠 */
            #${menu.id} details.zen-ai-reasoning{
                margin-bottom: 8px;
                padding: 8px 10px;
                background: #f8fafc;
                border: 1px solid #e2e8f0;
            ...
            #${menu.id} details.zen-ai-reasoning summary{
                color: #4f46e5;
                font-weight: 700;
                cursor: pointer;
                user-select: none;
            ...
            #${menu.id} details.zen-ai-reasoning summary:hover{
                text-decoration: underline;
            }
            #${menu.id} details.zen-ai-reasoning .reasoning-text{
                margin-top: 6px;
                padding-top: 6px;
            ...
            #${menu.id} .quick-fill-btn:hover{
                background: #e0e7ff;
                color: #4338ca;
            }
            #${menu.id} .zen-ai-content{ min-height:64px; padding:11px; color:#263247; font-size:12px; line-height:1.7; white-space:pre-wrap; overflow-wrap:anywhere; }
            #${menu.id} .zen-result-summary{ position:sticky; top:-10px; display:flex; align-items:center; justify-content:space-between; margin:-10px -10px 8px; padding:9px 11px; background:rgba(255,255,255,.96); border-bottom:1px solid #e8edf3; backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); z-index:2; }
            #${menu.id} .zen-result-summary strong{ font-size: 13px; }
            #${menu.id} .zen-result-summary span{ color:var(--zen-text-muted); font-size:11px; }
            #${menu.id} .zen-result-item{ margin-top:7px; overflow:hidden; background:#fff; border:1px solid #e2e8f0; border-radius:10px; box-shadow:0 2px 6px rgba(15,23,42,.04); }
            #${menu.id} .zen-result-section{ display:grid; grid-template-columns:34px minmax(0,1fr); gap:7px; padding:9px; }
            #${menu.id} .zen-answer-section{ background:#f8fafc; border-top:1px solid #edf1f5; }
            #${menu.id} .zen-result-label{ align-self:start; padding:2px 0; color:#64748b; font-size:10px; font-weight:700; text-align:center; background:#eef2ff; border-radius:5px; }
            #${menu.id} .zen-answer-section .zen-result-label{ color:#047857; background:#ecfdf5; }
            #${menu.id} .zen-result-content{ min-width:0; color:#263247; font-size:12px; line-height:1.65; white-space:pre-wrap; }
            #${menu.id} .zen-empty-state, #${menu.id} .zen-searching{ padding:20px 10px; color:var(--zen-text-muted); text-align:center; }
            #${menu.id} .zen-answer-btn{ display:inline-block; margin:0; color:var(--zen-primary); font-weight:600; cursor:pointer; }
            #${menu.id} .zen-correct-btn{ height:24px; min-width:38px; margin:4px 0 0 6px; padding:0 7px; font-size:11px; }
            #${menu.id} .zen-user-ans-tip{ color:#d97706; }
            ...
            #${menu.id} #settings_modal .modal-panel{
                ...
                border-radius: 18px;
                box-shadow: 0 25px 60px -12px rgba(15, 23, 42, .35);
                pointer-events: auto;
            ...
            #${menu.id} #settings_modal .modal-scroll-body{
                overflow-y: auto;
                padding-right: 4px;
                flex: 1;
            }
            #${menu.id} #settings_modal .modal-scroll-body::-webkit-scrollbar{ width: 5px; }
            #${menu.id} #settings_modal .modal-scroll-body::-webkit-scrollbar-thumb{ background: #cbd5e1; border-radius: 4px; }
            #${menu.id} #settings_modal .modal-panel .row{ margin: 10px 0; }
            ...
            #${menu.id} #settings_modal .modal-panel .row .tip-text{
                color: #94a3b8;
                font-size: 11px;
                font-weight: normal;
            }
            ...
            #${menu.id} #settings_modal .actions{
                display: flex;
            ...
            #${menu.id} #settings_cancel{
                background: #f8fafc;
                color: #475569;
            }
            #${menu.id} #settings_cancel:hover{
                background: #f1f5f9;
                color: #1e293b;
            }
        ...
        let $menu =
            ...
            <div id='${menu.id}'>
                <div id ="zhu_${menu.id}" >
                    <div id="x_set" title="单击展开面板，按住可拖拽移动">🦄</div>
                    <button id="x_zhu_auto" title="一键智能自动作答整页客观试题"><span>⚡ 自动答题</span></button>
                </div>
                <div class= "drawer" id="set">
                    <div class="drawer-header">
                        <div>
                            <strong>网课助手</strong>
                            <span class="clean-badge">AI 极速版</span>
                        </div>
                        <button id="x_settings" title="配置大模型与视频速率" aria-label="基本设置">⚙️ 设置</button>
                    </div>
                    <div class="account-row">
                        <span class="status-dot"></span>
                        <input id="ai_status_display" readonly="readonly" value="AI引擎：检查中..." />
                        <button id="quick_open_settings" class="secondary-btn" title="点击配置自定义API">配置</button>
                    </div>
                    <div class="search-row">
                        <span class="field-icon">⌕</span>
                        <input id="find_input" placeholder="输入题目或使用 Shift+↑ 截图识题..." />
                        <button id="x_find" class="primary-btn" title="题库检索">题库搜</button>
                        <button id="x_AIfind" class="ai-btn" title="调用自定义AI大模型求解">AI 解答</button>
                    </div>
                    <button id="x_auto_page" class="auto-answer-btn" title="自动扫描整页客观试题，调用配置的AI大模型智能分析并自动勾选答案">⚡ 一键全自动答题 (整页)</button>
                    <div class="drawer-footer">
                        <span>zensu357 · AI驱动</span>
                        <div class="footer-links">
                            <span style="color:#64748b;">Shift+↑ 截图识字</span>
                        </div>
                    </div>
                </div>
                ...
                <div id="settings_modal">
                    <div class="modal-panel">
                        <div class="modal-title">⚙️ 助手与自定义大模型设置</div>
                        <div class="modal-scroll-body">
                            <div class="row">
                                <label for="settings_ai_url">API 接口地址 (URL) <span class="tip-text">OpenAI兼容规范</span></label>
                                <input id="settings_ai_url" type="text" placeholder="如 https://api.deepseek.com/chat/completions" />
                            </div>
                            <div class="row">
                                <label for="settings_ai_key">API 密钥 (API Key) <span class="tip-text">本地安全存储</span></label>
                                <input id="settings_ai_key" type="password" placeholder="sk-xxxxxxxxxxxxxxxxxxxx" />
                            </div>
                            <div class="row">
                                <label for="settings_ai_model">模型名称 (Model) <span class="tip-text">如 deepseek-chat</span></label>
                                <input id="settings_ai_model" type="text" placeholder="deepseek-chat / gpt-4o-mini / qwen-plus" />
                            </div>
                            <div class="row">
                                <label for="settings_ai_prompt">系统提示词 (System Prompt)</label>
                                <textarea id="settings_ai_prompt" rows="3"></textarea>
                            </div>
                            <div class="row">
                                <label for="settings_video_spend">视频播放速率 (默认 1.0)</label>
                                <input id="settings_video_spend" type="number" step="0.1" min="0.5" max="16" />
                            </div>
                        </div>
                        <div class="actions">
                            <button id="settings_cancel">取消</button>
                            <button id="settings_save">保存配置</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        // 健壮化挂载：防止 DOM 尚未就绪时报错
        const parentDom = document.body || document.documentElement || document.getElementsByTagName('html')[0];
        if(!parentDom){
            setTimeout(() => this.initMenu(show), 500);
            return null;
        }
        const hostElement = parentDom.appendChild(document.createElement('div'));
        hostElement.id = "clean_assistant_container";
        ...
        try {
            ...
            try {
                ...
                const originalAttach = iframe.contentWindow.Element.prototype.attachShadow;
                document.documentElement.removeChild(iframe);
                shadowRoot = originalAttach.call(hostElement, { mode: 'closed' });
            } catch (e2) {
        ...
        const updateAiDisplay = () => {
            const curModel = GM_getValue("custom_ai_model") || "deepseek-chat";
            const hasKey = !!GM_getValue("custom_ai_api_key");
            const statusInput = shadowContent.querySelector("#ai_status_display");
            if(statusInput){
                statusInput.value = hasKey ? `AI引擎：${curModel} (已就绪)` : "AI引擎：未配置Key (点击配置)";
            }
        };
        updateAiDisplay();
        ...
        $(shadowContent).on('click', '#x_set', function (e) {
            if(e.currentTarget.dataset.zenDragged === "true"){
                e.currentTarget.dataset.zenDragged = "false";
                return;
            }
            if(document.querySelectorAll("#wzq").length>1 || document.querySelectorAll("#qptjb").length==1){
                alert('检测到您安装了多个不同版本的脚本，请关闭其他重复脚本以免发生冲突。');
            }
        ...
        $(shadowContent).on('click', '#x_find',async function () {
            ...
            if(find_input.value.length < 6){
                aner.html('<div class="zen-empty-state">请输入至少 6 个字符后再搜索</div>');
                $btn.prop('disabled', false);
                clearTimeout(t);
                return;
            }
            aner.html('<div class="zen-searching">正在检索题库，请稍候…</div>');
            await window.my.findproblem(find_input.value.replace(/   /g,"   ").substr(0,30));
            clearTimeout(t);
            $btn.prop('disabled', false);
            aner.text("");
            aner.append('<div class="zen-result-summary"><strong>搜索结果</strong><span>共 '+(window.my.config.answer && window.my.config.answer.rows ? window.my.config.answer.rows.length : 0)+' 条</span></div>');
            if(window.my.config.answer && window.my.config.answer.rows){
                window.my.config.answer.rows.forEach(row=>{
                    const item = $('<section class="zen-result-item"></section>');
                    item.append('<div class="zen-result-section"><span class="zen-result-label">题目</span><div class="zen-result-content">'+row.subject+'</div></div>');
                    const answer = $('<div class="zen-result-section zen-answer-section"><span class="zen-result-label">答案</span><div class="zen-result-content"></div></div>');
                    if(row.hideAnswer){
                        answer.find('.zen-result-content').append("<span class=\"zen-answer-btn\" data-problem-id=\""+row.problemId+"\">点击查看答案</span>");
                    }else{
                        answer.find('.zen-result-content').text(row.answers.join(' '));
                    }
                    item.append(answer);
                    aner.append(item);
                });
        ...
        $(shadowContent).on('click', '.zen-answer-btn', async function () {
            const el = $(this);
            ...
            if(user_ans){
                el.replaceWith('<span class="zen-user-ans-tip">'+text+'<br/><strong>纠正答案：</strong>'+user_ans+'</span>'+
                               '<button class="zen-correct-btn" data-problem-id="'+pid+'" data-create-user="'+(createBy||'')+'">纠正</button>');
            }else if(text){
        ...
        $(shadowContent).on('click', '.zen-correct-btn', async function () {
            const $btn = $(this);
            const pid = $btn.data('problem-id');
            const cu = $btn.data('create-user');
            const input = window.prompt('请输入纠正答案：','');
            if(!input){ return; }
            $btn.prop('disabled', true).text('提交中...');
            try{
                await window.my.upladApi('/tiku/api/v1/problemStatus', {
                    problemId: pid,
                    createUser: cu,
                    userAnswer: input,
                    status: "-1"
        ...
        // 核心改造：纯净自定义 AI 大模型搜题/问答 (强化错误拦截与状态反馈)
        $(shadowContent).on('click', '#x_AIfind', async function () {
            const $btn = $(this);
            const questionText = find_input.value.trim();
            aner.removeClass("question-result-view welcome-view").addClass("ai-result-view").show("slow");
            aner.text("");

            if(!questionText){
                aner.html('<div class="zen-empty-state">请输入需要询问 AI 的题目或内容</div>');
                return;
            }

            const apiUrl = GM_getValue("custom_ai_api_url") || "https://api.deepseek.com/chat/completions";
            const apiKey = GM_getValue("custom_ai_api_key") || "";
            const modelName = GM_getValue("custom_ai_model") || "deepseek-chat";
            const systemPrompt = GM_getValue("custom_ai_prompt") || "你是一位专业的大学网课与考试答题助手。请针对用户给出的题目（含单选、多选、判断、填空等），直接清晰准确地给出【正确答案与选项】，并提供简明扼要的解析。";

            if(!apiKey){
                aner.html(
                    '<div class="zen-empty-state" style="padding:24px 12px;">' +
                        '<div style="font-size:14px;font-weight:700;margin-bottom:8px;color:#1e293b;">🤖 尚未配置大模型 API Key</div>' +
                        '<div style="color:#64748b;font-size:12px;margin-bottom:16px;">本纯净版已全面支持自定义 AI 引擎，请先配置你的 API 密钥。</div>' +
                        '<button id="guide_open_settings" class="primary-btn" style="height:32px;padding:0 16px;">⚙️ 立即配置 API</button>' +
                        '<div style="margin-top:12px;font-size:11px;color:#94a3b8;">支持 DeepSeek、OpenAI、通义千问、Kimi 或本地 Ollama</div>' +
                    '</div>'
                );
                return;
            }
            ...
            $btn.prop('disabled', true);
            setTimeout(() => { $btn.prop('disabled', false); }, 2500);

            // 规范化接口 URL
            let normalizedUrl = apiUrl.trim();
            if(!normalizedUrl.endsWith("/chat/completions") && !normalizedUrl.includes("/api/chat")){
                normalizedUrl = normalizedUrl.replace(/\/+$/, "") + "/chat/completions";
            }

            aner.html(
                '<section class="zen-ai-card">' +
                    '<div class="zen-ai-header">' +
                        '<span class="zen-ai-icon">AI</span>' +
                        '<strong>'+ modelName +' 智能解答</strong>' +
                        '<span class="zen-ai-status">思考中…</span>' +
                    '</div>' +
                    '<details class="zen-ai-reasoning" style="display:none;">' +
                        '<summary class="zen-ai-reasoning-title">🧠 思考过程 (默认折叠，点击展开)</summary>' +
                        '<div class="reasoning-text"></div>' +
                    '</details>' +
                    '<div class="zen-ai-content"></div>' +
                '</section>'
            );

            const aiOutput = aner.find('.zen-ai-content');
            const aiStatus = aner.find('.zen-ai-status');
            const reasoningBox = aner.find('.zen-ai-reasoning');
            const reasoningText = aner.find('.reasoning-text');
            ...
            GM_xmlhttpRequest({
                ...
                onloadstart: function(response) {
                    // 检查 HTTP 状态码，如果出现 401/403/404/429/500 等立即报错反馈
                    if (response.status && (response.status < 200 || response.status >= 300)) {
                        aiStatus.text('请求出错 (' + response.status + ')').addClass('error');
                        const errReader = response.response ? response.response.getReader() : null;
                        if (errReader) {
                            errReader.read().then(({value}) => {
                                let errStr = new TextDecoder("utf-8").decode(value);
                                try {
                                    let errObj = JSON.parse(errStr);
                                    aiOutput.html('<div style="color:#dc2626;font-size:12px;">接口错误 [' + response.status + ']: ' + (errObj.error?.message || errObj.msg || errStr) + '</div>');
                                } catch(e) {
                                    aiOutput.html('<div style="color:#dc2626;font-size:12px;">接口错误 [' + response.status + ']: ' + errStr + '</div>');
                                }
                            }).catch(() => {
                                aiOutput.html('<div style="color:#dc2626;font-size:12px;">接口返回 HTTP ' + response.status + '，请检查 API Key 与模型名称是否正确。</div>');
                            });
                        } else {
                            aiOutput.html('<div style="color:#dc2626;font-size:12px;">接口返回 HTTP ' + response.status + '，请检查 API Key 与模型名称是否正确。</div>');
                        }
                    ...
                    function processStream() {
                        reader.read().then(({ done, value }) => {
                            if (done) {
                                aiStatus.text('解答完成').addClass('done');
                                reasoningBox.find('.zen-ai-reasoning-title').text('🧠 思考过程 (已折叠，点击展开)');
                                if(fullAiText && !aner.find('.quick-fill-container').length){
                                    const parsedChoice = extractAnswerChoice(fullAiText);
                                    aner.find('.zen-ai-card').append(
                                        '<div class="quick-fill-container" style="padding:8px 11px;background:#f8fafc;border-top:1px solid #edf1f5;display:flex;align-items:center;justify-content:space-between;">' +
                                            '<span style="font-size:11px;color:#64748b;">识别答案：<strong style="color:#059669;">' + parsedChoice + '</strong></span>' +
                                            '<button class="quick-fill-btn" data-ans="' + parsedChoice + '">⚡ 勾选到页面试题</button>' +
                                        '</div>'
                                    );
                                }
                                return;
                                ...
                                try {
                                    ...
                                    if(delta){
                                        if(delta.reasoning_content){
                                            reasoningBox.show();
                                            reasoningText.append(document.createTextNode(delta.reasoning_content));
                                        }
                                        if(delta.content){
                                            ...
                                            if(!inThink && chunk.includes("<think>")){
                                                inThink = true;
                                                reasoningBox.show();
                                                let parts = chunk.split("<think>");
                                                if(parts[0]) aiOutput.append(document.createTextNode(parts[0]));
                                                if(parts[1]) reasoningText.append(document.createTextNode(parts[1]));
                                            } else if(inThink){
                                                if(chunk.includes("</think>")){
                                                    inThink = false;
                                                    let parts = chunk.split("</think>");
                                                    if(parts[0]) reasoningText.append(document.createTextNode(parts[0]));
                                                    if(parts[1]) aiOutput.append(document.createTextNode(parts[1]));
                                                } else {
                                                    reasoningText.append(document.createTextNode(chunk));
                                                }
                                            } else {
                                                aiOutput.append(document.createTextNode(chunk));
                                            }
                            ...
                            aiStatus.text('流传输中断').addClass('error');
                            aiOutput.append("\n[数据读取中断: " + err + "]");
                        });
                    }
                    processStream();
                ...
                onerror: function(err) {
                    aiStatus.text('请求失败').addClass('error');
                    aiOutput.html('<div style="color:#dc2626;">请求失败，请检查网络或跨域设置。<br/>错误信息: ' + JSON.stringify(err) + '</div>');
                },
                ontimeout: function() {
                    aiStatus.text('请求超时').addClass('error');
                    aiOutput.html('<div style="color:#dc2626;">请求大模型接口超时，请检查接口地址是否可用。</div>');
                }
            });
        });
        ...
        function extractAnswerChoice(text) {
            ...
            const explicit = clean.match(/(?:答案|选择|正确选项|应选|正确答案)[:：\s*]*([A-G]+|正确|错误|对|错|√|×)/i);
            if (explicit) {
                let val = explicit[1].toUpperCase();
                if (val === '对' || val === '√') val = '正确';
                if (val === '错' || val === '×') val = '错误';
                return val;
            }
            ...
            const letterMatch = firstLine.match(/^[【\(\[]?([A-G]+)[\)\]】\.\s、:]/i) || firstLine.match(/^([A-G]+)$/i);
            if (letterMatch) return letterMatch[1].toUpperCase();

            if (firstLine.includes("正确") || firstLine.includes("对") || firstLine.includes("√") || firstLine.toUpperCase().includes("TRUE")) return "正确";
            if (firstLine.includes("错误") || firstLine.includes("错") || firstLine.includes("×") || firstLine.toUpperCase().includes("FALSE")) return "错误";

            const anyLetter = firstLine.match(/[A-G]+/i);
            if (anyLetter) return anyLetter[0].toUpperCase();

            return firstLine.slice(0, 15);
        }

        // 辅助：Promise 调用大模型解答单题
        function queryAiForQuestion(apiUrl, apiKey, modelName, promptText) {
            return new Promise((resolve, reject) => {
                ...
                GM_xmlhttpRequest({
                    ...
                    data: JSON.stringify({
                        model: modelName,
                        messages: [
                            { role: "system", content: "你是一位高校课程考试答题助手。请直接在第一行输出正确选项（如 A、B 或多选 ABC，或判断题 正确/错误），第二行写简明解析。" },
                    ...
                    onerror: function(e) { reject("网络连接失败"); },
                    ontimeout: function() { reject("请求超时"); }
                });
            });
        }

        // 辅助：递归获取页面所有同源文档（深度遍历主页面和所有嵌套 iframe）
        function getAllDocuments(rootDoc, depth, docs) {
            depth = depth || 0;
            docs = docs || [];
            if (!rootDoc || depth > 8) return docs;
            if (!docs.includes(rootDoc)) docs.push(rootDoc);
            try {
                ...
                for (let i = 0; i < iframes.length; i++) {
                    try {
                        ...
                        if (cDoc && !docs.includes(cDoc)) {
                            getAllDocuments(cDoc, depth + 1, docs);
                        }
                    } catch(e) {}
                }
            } catch(e) {}
        ...
        // 辅助：高穿透多层事件模拟点击（覆盖 原生click、MouseEvents、PointerEvents、input.checked、jQuery.trigger）
        function simulateClick(el) {
            ...
            try {
                if (typeof el.querySelectorAll === 'function') {
                    el.querySelectorAll('input[type="radio"], input[type="checkbox"], a, label, span, i').forEach(c => targets.add(c));
                }
                if (typeof el.closest === 'function') {
                    const pLabel = el.closest('label');
                    if (pLabel) targets.add(pLabel);
                    const pLi = el.closest('li');
                    if (pLi) targets.add(pLi);
                }
            } catch(e) {}
            ...
            targets.forEach(target => {
                ...
                // 2. 模拟真实指针与鼠标事件
                if (typeof MouseEvent !== 'undefined' && win) {
                ...
                // 4. 触发所属 Window 上下文的 jQuery click 事件
                if (win && win.$ && typeof win.$(target).trigger === 'function') {
                    try {
                        win.$(target).trigger('click');
                        win.$(target).trigger('change');
                    } catch(e){}
                }
            });
        }
        ...
        // 辅助：匹配并勾选具体选项
        function matchAndClickOption(optItem, answerChoice, qType) {
            const el = optItem.el;
            const text = (optItem.text || '').trim();
            const dataVal = (el.getAttribute('data') || el.getAttribute('value') || el.getAttribute('data-value') || '').trim();
            const letters = (answerChoice.match(/[A-G]/gi) || []).map(l => l.toUpperCase());

            // 1. 客观选择题（单选/多选）
            if (letters.length > 0) {
                for (const l of letters) {
                    if (dataVal.toUpperCase() === l) {
                        simulateClick(el);
                        return true;
                    }
                    const innerInput = el.querySelector ? el.querySelector('input') : null;
                    if (innerInput && (innerInput.value || '').trim().toUpperCase() === l) {
                        simulateClick(innerInput);
                        simulateClick(el);
                        return true;
                    }
                    const keyEl = el.querySelector ? el.querySelector('.after, .num_option, .option-key, .node-lab, i.fl, b') : null;
                    if (keyEl && keyEl.textContent.trim().toUpperCase() === l) {
                        simulateClick(keyEl);
                        simulateClick(el);
                        return true;
                    }
                    const prefixRegex = new RegExp('^[\\s\\(\\[\\{（【]*' + l + '[\\s\\)\\]\\}）】\\.\\、\\:\\：\\s]', 'i');
                    if (prefixRegex.test(text) || text.toUpperCase() === l) {
                        simulateClick(el);
                        return true;
                    }
                }
            ...
            // 2. 判断题
            if (qType === '判断题' || answerChoice.includes('正确') || answerChoice.includes('错误') || answerChoice.includes('对') || answerChoice.includes('错')) {
                const isTrue = answerChoice.includes('正确') || answerChoice.includes('对') || answerChoice.includes('√') || answerChoice.toUpperCase().includes('TRUE');
                const isFalse = answerChoice.includes('错误') || answerChoice.includes('错') || answerChoice.includes('×') || answerChoice.toUpperCase().includes('FALSE');

                const hasTrueText = text.includes('正确') || text.includes('对') || text.includes('√') || text.toUpperCase().includes('TRUE') || text === '是';
                const hasFalseText = text.includes('错误') || text.includes('错') || text.includes('×') || text.toUpperCase().includes('FALSE') || text === '否';

                const innerInput = el.querySelector ? el.querySelector('input') : null;
                const inputVal = innerInput ? (innerInput.value || '').toLowerCase() : '';

                if (isTrue && (hasTrueText || inputVal === 'true' || inputVal === '1' || (el.classList && el.classList.contains('ri')))) {
                    simulateClick(innerInput || el);
                    simulateClick(el);
                    return true;
                }
                if (isFalse && (hasFalseText || inputVal === 'false' || inputVal === '0' || (el.classList && el.classList.contains('wr')))) {
                    simulateClick(innerInput || el);
                    simulateClick(el);
                    return true;
                }
        ...
        // 辅助：从页面中扫描所有有效试题
        function scanPageQuestions(docList) {
            const questionItems = [];
            const seenElements = new Set();

            docList.forEach(d => {
                try {
                    const qElements = d.querySelectorAll(
                        '.TiMu, .questionLi, .subjectDet, .singleQuesId, ' +
                        'div[id^="question"], .Py_tk, .ans-cc, .aiArea, ' +
                        '.topic-item, .exam-item, .question-item, .problem-item, .test-item'
                    );
                    ...
                    qElements.forEach(el => {
                        ...
                        let title = '';
                        const titleSelectors = ['.Zy_TItle', '.mark_name', '.newZy_TItle', 'h5', '.subject-title', '.topic-title', '.title', 'h3', 'h4'];
                        ...
                        if (!title) title = (el.textContent || '').trim().slice(0, 80);
                        ...
                        let qType = '单选题';
                        if (wholeText.includes('多选题') || wholeText.includes('多选')) qType = '多选题';
                        else if (wholeText.includes('判断题') || wholeText.includes('判断')) qType = '判断题';
                        else if (wholeText.includes('填空题') || wholeText.includes('填空')) qType = '填空题';

                        const optList = [];
                        const optEls = el.querySelectorAll(
                            'ul.Zy_ulTop li, .answerBg, .answer_p, ' +
                            'label.el-radio, label.el-checkbox, .el-radio, .el-checkbox, ' +
                            'li.clearfix, label:has(input), ' +
                            'div[class*="option"], .answer-item'
                        );

                        const seenOpt = new Set();
                        optEls.forEach(oEl => {
                            if (seenOpt.has(oEl)) return;
                            const t = (oEl.textContent || '').trim();
                            const inps = oEl.querySelectorAll ? oEl.querySelectorAll('input') : [];
                            if (t || inps.length > 0) {
                                seenOpt.add(oEl);
                                optList.push({ el: oEl, text: t });
                            }
                        });

                        if (title.length >= 2 && (optList.length > 0 || qType === '判断题')) {
                            seenElements.add(el);
                            questionItems.push({
                                rawElement: el,
                                $el: $(el),
        ...
        // 辅助：在页面中查找某道题并勾选答案（单题勾选回填）
        function clickAnswerOnPage(questionTitle, answerStr) {
            const docs = getAllDocuments(document);
            const questions = scanPageQuestions(docs);
            let found = false;

            for (const q of questions) {
                if (q.title.includes(questionTitle.slice(0, 8)) || questionTitle.includes(q.title.slice(0, 8))) {
                    for (const opt of q.options) {
                        if (matchAndClickOption(opt, answerStr, q.type)) {
                            found = true;
                        }
                    }
                    if (found) break;
        ...
        $(shadowContent).on('click', '.quick-fill-btn', function() {
            const ans = $(this).data('ans');
            const qTitle = find_input.value.trim();
            const filled = clickAnswerOnPage(qTitle, ans);
            if (filled) {
                $(this).text('✅ 已勾选页面试题').prop('disabled', true);
            } else {
                $(this).text('⚠️ 未在页面找到该题选项');
                setTimeout(() => { $(this).text('⚡ 勾选到页面试题'); }, 2000);
            }
        });
        ...
        const autoAnswerEngine = {
            ...
            addLog: function(msg, level) {
                ...
                const $term = $(shadowContent).find('#auto_log_terminal');
                if ($term.length) {
                    let tagHtml = `<span class="log-tag-${level}">[${level.toUpperCase()}]</span>`;
                    if (level === 'sys') tagHtml = `<span class="log-tag-sys">[系统]</span>`;
                    else if (level === 'succ') tagHtml = `<span class="log-tag-succ">[成功]</span>`;
                    else if (level === 'warn') tagHtml = `<span class="log-tag-warn">[提示]</span>`;
                    else if (level === 'err') tagHtml = `<span class="log-tag-err">[错误]</span>`;
                    else if (level === 'ai') tagHtml = `<span class="log-tag-ai">[AI]</span>`;

                    $term.append(`<div class="log-item"><span class="log-time">${timeStr}</span> ${tagHtml} ${msg}</div>`);
                    const autoScroll = $(shadowContent).find('#auto_log_autoscroll').prop('checked');
                    if (autoScroll !== false) {
            ...
            clearLog: function() {
                $(shadowContent).find('#auto_log_terminal').empty();
            },

            updateBadge: function(text, className) {
                const $b = $(shadowContent).find('#auto_status_badge');
                $b.attr('class', 'auto-badge ' + className).text(text);
            },

            updateStats: function() {
                $(shadowContent).find('#auto_stat_total').text(this.totalCount);
                $(shadowContent).find('#auto_stat_current').text(this.currentIndex);
                $(shadowContent).find('#auto_stat_success').text(this.successCount);
                $(shadowContent).find('#auto_stat_fail').text(this.failCount);

                const pct = this.totalCount > 0 ? Math.round((this.currentIndex / this.totalCount) * 100) : 0;
                $(shadowContent).find('#auto_progress_bar').css('width', pct + '%');
                $(shadowContent).find('#auto_progress_text').text('进度: ' + pct + '%');
            },

            pause: function() {
                if (!this.isRunning || this.isPaused) return;
                this.isPaused = true;
                this.updateBadge('🟡 已暂停', 'paused');
                const $btn = $(shadowContent).find('#auto_btn_pause');
                $btn.html('▶️ 继续答题').removeClass('btn-pause').addClass('btn-resume');
                this.addLog('答题流程已暂停。点击【继续】按钮可恢复作答。', 'warn');
            },

            resume: function() {
                if (!this.isRunning || !this.isPaused) return;
                this.isPaused = false;
                this.updateBadge('🟢 作答中', 'running');
                const $btn = $(shadowContent).find('#auto_btn_pause');
                $btn.html('⏸️ 暂停答题').removeClass('btn-resume').addClass('btn-pause');
                this.addLog('答题流程恢复，继续推进...', 'sys');
            },
            ...
            stop: function() {
                ...
                this.isPaused = false;
                this.updateBadge('🔴 已停止', 'stopped');
                $(shadowContent).find('#auto_btn_pause').prop('disabled', true);
                $(shadowContent).find('#auto_btn_stop').prop('disabled', true);
                $(shadowContent).find('#auto_btn_restart').prop('disabled', false);
                if (this.timerId) clearInterval(this.timerId);
                this.addLog('用户手动终止了自动答题任务。', 'warn');
            }
        };

        // 核心：全自动答题引擎（扫描本页所有试题、AI作答并自动勾选选项）
        async function runAutoAnswer() {
            div_set.hide();
            aner.removeClass("ai-result-view welcome-view").addClass("question-result-view auto-mode").show("fast");
            ...
            if(!apiKey){
                aner.html(
                    '<div class="zen-empty-state" style="padding:24px 12px;text-align:center;">' +
                        '<div style="font-weight:700;margin-bottom:8px;color:#dc2626;font-size:14px;">🤖 尚未配置大模型 API Key</div>' +
                        '<div style="font-size:12px;color:#64748b;margin-bottom:16px;">自动答题需要调用大模型智能判断选项，请先在设置中填写你的 API Key。</div>' +
                        '<button id="guide_open_settings_auto" class="primary-btn" style="height:32px;padding:0 16px;">⚙️ 立即配置 API</button>' +
                    '</div>'
                );
                $('#guide_open_settings_auto').click(() => {
                    const modal = $(shadowContent.querySelector('#settings_modal'));
                    modal.show();
                    syncSettingsModal();
                });
            ...
            aner.html(
                '<div class="auto-console">' +
                    '<div class="auto-console-header">' +
                        '<div class="auto-console-title">' +
                            '<span>⚡ AI 智能自动答题控制台</span>' +
                            '<span id="auto_status_badge" class="auto-badge running">🟢 扫描试题...</span>' +
                        '</div>' +
                        '<button id="auto_back_btn" class="auto-back-btn" title="返回搜索主面板">返回主面板</button>' +
                    '</div>' +
                    '<div class="auto-stat-grid">' +
                        '<div class="auto-stat-card">' +
                            '<div class="num" id="auto_stat_total">0</div>' +
                            '<div class="label">试题总数</div>' +
                        '</div>' +
                        '<div class="auto-stat-card">' +
                            '<div class="num" id="auto_stat_current">0</div>' +
                            '<div class="label">当前题号</div>' +
                        '</div>' +
                        '<div class="auto-stat-card">' +
                            '<div class="num" id="auto_stat_success" style="color:#059669;">0</div>' +
                            '<div class="label">成功勾选</div>' +
                        '</div>' +
                        '<div class="auto-stat-card">' +
                            '<div class="num" id="auto_stat_fail" style="color:#dc2626;">0</div>' +
                            '<div class="label">跳过/异常</div>' +
                        '</div>' +
                    '</div>' +
                    '<div class="auto-progress-bar-wrap">' +
                        '<div id="auto_progress_bar" class="auto-progress-bar-fill"></div>' +
                    '</div>' +
                    '<div style="display:flex;justify-content:space-between;font-size:11px;color:#64748b;margin-top:-4px;">' +
                        '<span id="auto_progress_text">进度: 0%</span>' +
                        '<span id="auto_time_text">耗时: 0s</span>' +
                    '</div>' +
                    '<div class="auto-console-toolbar">' +
                        '<button id="auto_btn_pause" class="console-btn btn-pause">⏸️ 暂停答题</button>' +
                        '<button id="auto_btn_stop" class="console-btn btn-stop">⏹️ 停止答题</button>' +
                        '<button id="auto_btn_restart" class="console-btn btn-restart">🔄 重新扫描</button>' +
                        '<button id="auto_btn_clear_log" class="console-btn btn-clear">🗑️ 清空</button>' +
                    '</div>' +
                    '<div class="terminal-bar">' +
                        '<div class="terminal-dots">' +
                            ...
                            '<span class="terminal-title">实时运行终端日志</span>' +
                        '</div>' +
                        '<label class="autoscroll-label">' +
                            '<input type="checkbox" id="auto_log_autoscroll" checked="checked" /> 自动滚动' +
                        '</label>' +
                    '</div>' +
                    '<div id="auto_log_terminal" class="auto-log-terminal"></div>' +
                '</div>'
            );

            // 初始化引擎状态
            if (autoAnswerEngine.timerId) clearInterval(autoAnswerEngine.timerId);
            autoAnswerEngine.isRunning = true;
            autoAnswerEngine.isPaused = false;
            autoAnswerEngine.shouldStop = false;
            autoAnswerEngine.totalCount = 0;
            autoAnswerEngine.currentIndex = 0;
            autoAnswerEngine.successCount = 0;
            autoAnswerEngine.failCount = 0;
            autoAnswerEngine.startTime = Date.now();
            autoAnswerEngine.updateStats();

            autoAnswerEngine.timerId = setInterval(() => {
                if (!autoAnswerEngine.isRunning) return;
                if (!autoAnswerEngine.isPaused) {
                    const elapsed = Math.round((Date.now() - autoAnswerEngine.startTime) / 1000);
                    $(shadowContent).find('#auto_time_text').text('耗时: ' + elapsed + 's');
                }
            }, 1000);

            autoAnswerEngine.addLog(`🚀 启动自动答题引擎，当前模型：<strong style="color:#818cf8;">${modelName}</strong>`, 'sys');
            autoAnswerEngine.addLog('🔍 正在深度递归扫描主页面及所有嵌套 iframe...', 'sys');

            const docs = getAllDocuments(document);
            autoAnswerEngine.addLog(`共检测到 ${docs.length} 个 DOM 框架层级，开始提取试题...`, 'sys');

            const questions = scanPageQuestions(docs);
            autoAnswerEngine.totalCount = questions.length;
            autoAnswerEngine.updateStats();

            if (questions.length === 0) {
                autoAnswerEngine.isRunning = false;
                autoAnswerEngine.updateBadge('⚠️ 未找到试题', 'stopped');
                autoAnswerEngine.addLog('⚠️ 未在当前页面及所有嵌套框架中检测到试题容器。请确认已进入章节测验或作业做题界面。', 'warn');
                $(shadowContent).find('#auto_btn_pause, #auto_btn_stop').prop('disabled', true);
                return;
            }

            autoAnswerEngine.updateBadge(`作答中 (0/${questions.length})`, 'running');
            autoAnswerEngine.addLog(`✅ 成功定位到 <strong>${questions.length}</strong> 道有效试题，开始逐题分析答题...`, 'succ');

            for (let i = 0; i < questions.length; i++) {
                if (autoAnswerEngine.shouldStop) {
                    break;
                }

                while (autoAnswerEngine.isPaused) {
                    if (autoAnswerEngine.shouldStop) break;
                    await new Promise(r => setTimeout(r, 300));
                }
                if (autoAnswerEngine.shouldStop) break;

                const item = questions[i];
                autoAnswerEngine.currentIndex = i + 1;
                autoAnswerEngine.updateStats();
                autoAnswerEngine.updateBadge(`作答中 (${i + 1}/${questions.length})`, 'running');

                const promptText =
                    `请解答以下题目：\n` +
                    `题型：${item.type}\n` +
                    `题目：${item.title}\n` +
                    (item.options.length > 0 ? `选项：\n${item.options.map(o => o.text).join('\n')}\n` : '') +
                    `要求：第一行必须直接给出正确选项（如 A、B 或多选 ABC，或判断题 正确/错误）；第二行可写简要解析。`;

                autoAnswerEngine.addLog(`▶ [${i + 1}/${questions.length}] 【${item.type}】${item.title.slice(0, 24)}...`, 'sys');

                try {
                    const rawAnswer = await queryAiForQuestion(apiUrl, apiKey, modelName, promptText);
                    const parsedAns = extractAnswerChoice(rawAnswer);
                    autoAnswerEngine.addLog(`&nbsp;&nbsp;🤖 AI判断：<strong style="color:#34d399;">${parsedAns}</strong>`, 'ai');

                    let clicked = false;
                    for (const opt of item.options) {
                        if (matchAndClickOption(opt, parsedAns, item.type)) {
                            clicked = true;
                        }
                    }

                    if (clicked) {
                        autoAnswerEngine.successCount++;
                        autoAnswerEngine.addLog(`&nbsp;&nbsp;🎯 选项已成功勾选！`, 'succ');
                    } else {
                        autoAnswerEngine.failCount++;
                        autoAnswerEngine.addLog(`&nbsp;&nbsp;⚠️ 选项未匹配或无需勾选，请核对。`, 'warn');
                    }

                    item.$el.find('.show_answer').remove();
                    item.$el.append(
                        `<div class="show_answer" style="color:#047857;font-weight:700;margin:6px 0;font-size:12px;background:#ecfdf5;padding:5px 9px;border-radius:6px;border-left:3px solid #10b981;">` +
                            `✅ AI自动作答：${parsedAns}` + (clicked ? ' (已自动勾选)' : ' (请核验)') +
                        `</div>`
                    );
                } catch(err) {
                    autoAnswerEngine.failCount++;
                    autoAnswerEngine.addLog(`&nbsp;&nbsp;❌ 答题失败: ${err}`, 'err');
                }
            ...
            if (autoAnswerEngine.shouldStop) {
                autoAnswerEngine.updateBadge('🔴 已停止', 'stopped');
                autoAnswerEngine.addLog('⏹️ 答题已停止。', 'warn');
            } else {
                autoAnswerEngine.updateBadge('✅ 已完成', 'completed');
                autoAnswerEngine.addLog(`🎉 <strong>本页全部答题完毕！</strong> 共 ${questions.length} 题，成功勾选 ${autoAnswerEngine.successCount} 题。请检查核对后手动保存或提交！`, 'succ');
            }
        ...
        // 控制台按钮事件绑定
        $(shadowContent).on('click', '#auto_btn_pause', function() {
            if (autoAnswerEngine.isPaused) {
                autoAnswerEngine.resume();
            } else {
                autoAnswerEngine.pause();
            }
        });

        $(shadowContent).on('click', '#auto_btn_stop', function() {
            autoAnswerEngine.stop();
        });

        $(shadowContent).on('click', '#auto_btn_restart', function() {
            runAutoAnswer();
        });

        $(shadowContent).on('click', '#auto_btn_clear_log', function() {
            autoAnswerEngine.clearLog();
        });

        $(shadowContent).on('click', '#auto_back_btn', function() {
            aner.hide("fast");
            div_set.show("fast");
        });

        $(shadowContent).on('click', '#x_auto_page, #x_zhu_auto, #x_start', runAutoAnswer);
        this.autoAnswerAll = runAutoAnswer;
        if (typeof window !== 'undefined') { if (!window.my) window.my = this; window.my.autoAnswerAll = runAutoAnswer; }

        $(shadowContent).on('click', '#quick_open_settings, #guide_open_settings', function() {
            div_set.show();
            const modal = $(shadowContent.querySelector('#settings_modal'));
            modal.show();
            syncSettingsModal();
        ...
        $(shadowContent).on('click', '#x_yue', async function () {
            ...
            if(!answers||JSON.stringify(answers) == "{}"){
                aner.text("暂时没有此试卷信息。可直接开始答题进行搜索。");
                return;
            }
        ...
        $(shadowContent).on('click', '#x_forum', async function () {
            let texts = prompt("请添加讨论词，使用英文逗号,进行分隔\n注意最后不要加上,", GM_getValue("forum_texts") || "我是一号讨论词,我是二号讨论词");
            if(texts !== null){
        ...
        const syncSettingsModal = () => {
            $(shadowContent.querySelector('#settings_ai_url')).val(GM_getValue('custom_ai_api_url') || 'https://api.deepseek.com/chat/completions');
            $(shadowContent.querySelector('#settings_ai_key')).val(GM_getValue('custom_ai_api_key') || '');
            $(shadowContent.querySelector('#settings_ai_model')).val(GM_getValue('custom_ai_model') || 'deepseek-chat');
            $(shadowContent.querySelector('#settings_ai_prompt')).val(GM_getValue('custom_ai_prompt') || '你是一位专业的大学网课与考试答题助手。请针对用户给出的题目（含单选、多选、判断、填空等），直接清晰准确地给出【正确答案与选项】，并提供简明扼要的解析。');
        ...
        $(shadowContent).on('click', '#settings_save', function () {
            const url = $(shadowContent.querySelector('#settings_ai_url')).val().trim();
            const key = $(shadowContent.querySelector('#settings_ai_key')).val().trim();
            const model = $(shadowContent.querySelector('#settings_ai_model')).val().trim();
            const prompt = $(shadowContent.querySelector('#settings_ai_prompt')).val().trim();
            const v = parseFloat($(shadowContent.querySelector('#settings_video_spend')).val());

            GM_setValue('custom_ai_api_url', url || 'https://api.deepseek.com/chat/completions');
            GM_setValue('custom_ai_api_key', key);
            GM_setValue('custom_ai_model', model || 'deepseek-chat');
            GM_setValue('custom_ai_prompt', prompt);
            ...
            $(shadowContent.querySelector('#settings_modal')).hide();
            aner.removeClass("ai-result-view welcome-view").addClass("question-result-view").show("slow");
            aner.html('<div class="zen-empty-state" style="color:#059669;font-weight:700;">✅ 配置已保存成功！现在可以直接使用 AI 解答功能。</div>');
        });

        $(document).on('click', '.addpro', async function (_this) {
            $(_this.currentTarget).attr("class","delpro").css("background","#f2ffc9").text("移出错题集");
            let error_problem = GM_getValue(window.my.config.id+"_error_problem");
            if(!error_problem){ error_problem = {}; }
        ...
        // 四重安全赋值：确保局部闭包变量、window、_this 同步拥有 DOM 引用，彻底解决 undefined.append 报错
        aner = window.aner = _this.aner = $(shadowContent.querySelector("#aner"));
        div_zhu = window.div_zhu = _this.div_zhu = $(shadowContent.querySelector("#zhu_"+menu.id));
        div_set = window.div_set = _this.div_set = $(shadowContent.querySelector("#set"));
        find_input = window.find_input = _this.find_input = shadowContent.querySelector("#find_input");
...
(function(_this) {
    let $ = document.getElementById("yl_8") && document.getElementById("yl_8").onclick || _this.y$ || jQuery;
    var jietu_onload, jietu_mouse;
    var canvasImg
    var select_box
    var worker
    var offset ={
                ...
                worker.recognize(value,'chi_sim', {
                    ...
                    if (text) {
                        ...
                        try {
                            ...
                            if (forbears_window && forbears_window.div_set) {
                                forbears_window.div_set.parent().show();
                                forbears_window.div_set.show();
                                if (forbears_window.find_input) {
                                    forbears_window.find_input.value = text;
                                }
                            }
                        } catch(topErr) {
                            if (window.find_input) { window.find_input.value = text; }
                        }
                    }else{
